...
variabile.
Il setter della property deve effettuare l’assegnamento solo se il valore da assegnare
...
class C:
    ...
    def aggiungiProprieta(cls):
        lista=list(vars(cls).items())
        for nomeVariabile,valore in lista:
            if(type(valore)==str):
                nuovaNomeVariabile=nomeVariabile
                nuovaPrivateName="_"+nuovaNomeVariabile
                setattr(cls,nuovaPrivateName,valore)
                def getter(cls):
                    return getattr(cls,nuovaPrivateName)

                def setter(cls,valore):
                    if (type(valore) is str):
                        setattr(cls,nuovaPrivateName, valore)
                    else:
                        raise TypeError(f"Non e`possibile assegnare {valore} alla variabile {nuovaNomeVariabile}")

                setattr(cls, nuovaNomeVariabile, property(fget=getter, fset=setter))
...
c.z=anna. Tutto ok.
c.x=[1, 2]. Tutto ok.
Non e`possibile assegnare 3 alla variabile z ed e` corretto che sia cosi`
...
Ora assegnamo a D variabili di classe omonime a quelle di C ma con valori diversi
d.z={1, 4}. Tutto ok.
Non e`possibile assegnare [1, 2] alla variabile w ed e` corretto che sia cosi`
d.w=pop. Tutto ok.
Non e`possibile assegnare 6 alla variabile w ed e` corretto che sia cosi`
Non e`possibile assegnare 3 alla variabile n ed e` corretto che sia cosi`
