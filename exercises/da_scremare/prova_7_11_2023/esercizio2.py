'''
Scrivere nel file esercizio2.py un decorator factory decFact che prende in input un tipo t e
restituisce un decoratore di classe decorator che dota la classe decorata di un metodo di istanza
addToList che restituisce una lista contenente i nomi e i valori delle variabili di tipo t
dell’istanza per cui è invocato. Ciascuna coppia (nome,valore) deve essere all'interno di una
tupla. Il codice della classe non deve essere modificato.
'''

def decoratorFact(tipo):

	def decorator(cls):
		def addToList(self):
		# Check if the first argument is an instance of the specified class
			instance_vars = []
			for key, value in self.__dict__.items():
				if isinstance(value, tipo):
					instance_vars.append((key, value))
