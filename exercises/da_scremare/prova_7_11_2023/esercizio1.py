'''
Scrivere nel file esercizio1.py un decorator factory DecoratorFactF che prende in input un
valore v e restituisce un decoratore di funzione che modifica il comportamento della
funzione decorata come segue:
• se la funzione è invocata con tutti gli argomenti di tipo uguale a quello di v allora la
funzione restituisce lo stesso valore che avrebbe restituito la funzione originaria;
• se uno o piu` argomenti sono di tipo diverso da quello di v allora la funzione
restituisce il valore che la funzione originaria avrebbe restituito se fosse stata
invocata solo su tutti gli argomenti dello stesso tipo di v;
• se la funzione originaria lancia un’eccezione, questa deve essere lanciata anche dalla funzione
decorata.).
'''
...
def DecoratorFactF(v):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            newKwargs = {}
            for item in args:
                if isinstance(item, type(v)):
                    newArgs.append(item)
                else:
                    counter += 1 # trovato argomento che non è del tipo v
            for key, value in kwargs.items():
                if isinstance(value, type(v)):
                    newKwargs[key] = value
                else:
                    counter += 1 # trovato argomento che non è del tipo v
            return_value = None
